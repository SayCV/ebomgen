...
func parsePCBTextParts(filename string) (map[string]types.EBOMItem, error) {
	...
	for {
		...
		if err == nil && strVal == "*PARTTYPE*   ITEMS" {
			for range [6]int{} {
			...
			for {
				...
				if pkgName == "" && newspacelines == 1 {
					kv := strings.Fields(strVal)
					pkgName = kv[0]
					partDesc = kv[1]
					pkgDesc.References = []string{pkgName}
					pkgDesc.Desc = partDesc
					newspacelines = 0
				}
				for {
					byteVal, _, err = reader.ReadLine()
				...
				pkgList[pkgName] = pkgDesc
				//print(partName, flush = True)
				pkgName = ""
			}
		} else if err == nil && strVal == "*PART*       ITEMS" {
			for range [6]int{} {
				byteVal, _, err = reader.ReadLine()
			}
			strVal = strings.TrimSpace(string(byteVal))
...
// ExtractPADSPCBComponents Load a PADS PCB file and extract the part information
func ExtractPADSPCBComponents(filename string) ([]types.EBOMItem, error) {
	...
	parts, _ := parsePCBTextParts(filename)
